...
class ReLU(Activation):
    ...
    def activation_prime(self, x):
        return np.where(x > np.float32(0), np.float32(1), np.float32(0))


class LeakyReLU(Activation):
    def __init__(self, slope=0.01):
        super().__init__()
        self.slope = np.float32(slope)

    def activation(self, x):
        return np.where(x > np.float32(0), x, self.slope * x)

    def activation_prime(self, x):
        return np.where(x > np.float32(0), np.float32(1), self.slope)
